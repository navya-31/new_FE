...
function ItemCard({ item, tenants, setCurrentId }) {
  ...
  return (
    ...
    <Card className={classes.card} raised elevation={6}>
      <CardMedia className={classes.media} image={cardImage}></CardMedia>

      <CardContent className={classes.overlay}>
        <Typography gutterBottom variant="h5" component="div">
          {item.name} For Dinner and Lunch
        </Typography>
        <Typography variant="body2">Sides: Rice, Lentils, Sides</Typography>
        <Typography variant="body2">
          Price per serving: {item.unit_price} $
        </Typography>
      </CardContent>
    </Card>
    // </Box>
